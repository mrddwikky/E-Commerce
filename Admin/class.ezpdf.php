...
var $ez=array('fontSize'=>240); // used for storing most of the page configuration parameters
...
function ezStartPageNumbers($x,$y,$size,$pos='left',$pattern='',$num=''){
...
function ezStartText($x,$y,$size,$pos='left',$pattern='',$num=''){
  // put page numbers on the pages from here.
  // place then on the 'pos' side of the coordinates (x,y).
  // pos can be 'left' or 'right'
  // use the given 'pattern' for display, where (PAGENUM} and {TOTALPAGENUM} are replaced
...
function ezStartText2($x,$y,$size,$pos='left',$pattern='',$num=''){
  // put page numbers on the pages from here.
  // place then on the 'pos' side of the coordinates (x,y).
  // pos can be 'left' or 'right'
  // use the given 'pattern' for display, where (PAGENUM} and {TOTALPAGENUM} are replaced
  ...
  if (!$pattern || !strlen($pattern) and $_GET[name_manifest]){
    $pattern='Name Manifest : '.$_GET[name_manifest];
  }
  if (!isset($this->ez['pageNumbering'])){
    $this->ez['pageNumbering']=array();
  }
